...
app.controller('AppCtrl', function($scope, $filter, dataService, scopeDService) {
    ...
    $scope.pdf_make_init = function() {
        ...
        var titel = 'Testdokument';

        var patient_adress = '';
        patient_adress = patient_adress + $scope.d.dataMain.patient.data.ansprache + '\n';
        patient_adress = patient_adress + $scope.d.dataMain.patient.data.last_name + ' ' + $scope.d.dataMain.patient.data.first_name + '\n';
        ...
        var patient = $scope.d.dataMain.patient.data.ansprache + ' ' + $scope.d.dataMain.patient.data.last_name + ' ' + $scope.d.dataMain.patient.data.first_name + ' (' + $scope.d.dataMain.patient.data.extras.birthday_age + ')';

        var klinik = $scope.d.dataMain.config.data.customer.contact.name + '\n' + $scope.d.dataMain.config.data.customer.contact.slogan;

        $scope.d.docDefinition = {
            "header": {
                "columns": [
                    { text: patient, alignment: 'left', margin: [5, 5, 10, 20] },
                    { text: klinik, alignment: 'right', margin: [10, 5, 5, 20], fontSize: 10 }
                ]
            },
            "footer": {
                "columns": [
