...
angular.module('optinomicDataModule').controller('MainCtrl', function($scope, $q, $filter, $mdToast, $window, apiService, dataService, scopeDService, simpleStatistics) {
    ...
    $scope.d.functions.getDefaultExportSettings = function(app_id) {
        ...
        var response_query = minimal_query + " LIMIT 1;";

        // Querys - current patient
        var patient_query = "";
        var current_patient_where = "\nAND patient='%current_pid%';";
        current_patient_where = current_patient_where.replace("%current_pid%", parseInt(helpers.getPatientID()));
        var current_patient_name = $scope.dataMain.patient.data.extras.name;


        // Push - Default - Querys
        var data_package = {};
        data_package = {
            name: 'Minimal | all Patients',
            sql: minimal_query
        };
        ...
        data_package = {
            name: 'Minimal | ' + current_patient_name,
            sql: patient_query
        };

        sql_box.packages.push(data_package);
        data_package = {
            name: 'Response | all Patients',
            sql: base_query
        };
        sql_box.packages.push(data_package);
        ...
        sql_box.packages.push(data_package);
        data_package = {
            name: 'Response | ' + current_patient_name,
            sql: patient_query
        };
        sql_box.packages.push(data_package);
        ...
        api.success(function(data) {
            ...
            var data_package = {};
            data_package = {
                name: 'Response | Patient (all response fields)',
                sql: enhanced_query
            };
            sql_box.packages.push(data_package);
            ...
            sql_box.packages.push(data_package);
            data_package = {
                name: 'Response | ' + current_patient_name + ' (all response fields)',
                sql: patient_query
            };
            sql_box.packages.push(data_package);
