...
angular.module('optinomicDataModule').controller('MainCtrl', function($scope, $q, $filter, $mdToast, $window, apiService, dataService, scopeDService, simpleStatistics) {
    ...
    $scope.d.functions.getDefaultExportSettings = function(app_id, app_packages) {
        ...
        api.success(function(data) {
            ...
            patient_query = enhanced_query.replace(";", date_filter_where);
            data_package = {
                name: 'Response (full) | all Patients | date-range',
                sql: patient_query
            };
            sql_box.packages.push(data_package);
            ...
            patient_query = patient_query.replace(";", date_filter_where);
            data_package = {
                name: 'Response (full) | ' + current_patient_name + ' | date-range',
                sql: patient_query
            };
            sql_box.packages.push(data_package);
