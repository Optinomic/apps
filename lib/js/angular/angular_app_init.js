...
angular.module('optinomicDataModule').controller('VisaDialogController', function($scope, $mdDialog) {
    $scope.hide = function() {
        $mdDialog.hide();
    };
...
angular.module('optinomicDataModule').controller('MainCtrl', function($scope, $q, $filter, $mdToast, $mdDialog, $mdMedia, $window, apiService, dataService, scopeDService, simpleStatistics) {
    ...
    $scope.d.functions.visa = function(email, password) {
        ...
        api_call.success(function(data) {
            ...
            //console.log('===> Success - visa   ', return_object, deferred);
        });
        api_call.error(function(error) {
            ...
            //console.log('===> ERROR! visa   ', return_object, deferred);
        });
    ...
    //    $mdDialog.show({
    //            controller: VisaDialogController,
    //            template: '<md-dialog aria-label="Visa" ng-cloak> <form> <md-toolbar> <div class="md-toolbar-tools"> <h2>Mango (Fruit)</h2> <span flex></span> <md-button class="md-icon-button" ng-click="cancel()"> - </md-button> </div></md-toolbar> <md-dialog-content> <div class="md-dialog-content"> <h2>Username</h2> <md-input-container flex class="md-block"> <label>Passwort</label> <input name="password" ng-model="password"> </md-input-container> </div></md-dialog-content> <md-dialog-actions layout="row"> <md-button ng-click="cancel()"> Cancel </md-button> <md-button ng-click="answer(password)" style="margin-right:20px;"> Check </md-button> </md-dialog-actions> </form></md-dialog>',
    //            targetEvent: ev,
    //            clickOutsideToClose: true,
    //            fullscreen: useFullScreen
    //        })
    //        .then(function(answer) {
    //            $scope.status = 'You said the password was "' + answer + '".';
    //        }, function() {
