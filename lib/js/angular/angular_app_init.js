...
angular.module('optinomicDataModule').controller('MainCtrl', function($scope, $filter, dataService, scopeDService, simpleStatistics) {
    ...
    $scope.d.functions.toggleToolPanel = function() {
        ...
        if (visible) {
            $scope.d.grid.ptions.api.showToolPanel(false);
        } else {
            ...
            for (var property in result) {
                if (result.hasOwnProperty(property)) {
                    ...
                    if ((property === 'datestamp') || (property === 'startdate') || (property === 'submitdate')) {
                        ...
                        result[propertyName] = $filter("amDateFormat")(date, 'Do. MMMM YYYY');

                        var propertyName = property + '_day_full';
                        result[propertyName] = $filter("amDateFormat")(date, 'dddd, Do MMMM YYYY');
    ...
    $scope.d.functions.createColumnDefs = function(surveyResults, editable) {
        ...
        for (var property in result) {
            if (result.hasOwnProperty(property)) {
                ...
                if ((property === 'datestamp_year') || (property === 'startdate_year') || (property === 'submitdate_year')) {
                    line.width = 145;
                    line.hide = true;
                };

                if ((property === 'datestamp_week') || (property === 'startdate_week') || (property === 'submitdate_week')) {
                    line.width = 145;
                    line.hide = true;
                };

                if ((property === 'datestamp_day') || (property === 'startdate_day') || (property === 'submitdate_day')) {
                    line.width = 145;
                    line.hide = true;
                };

                if ((property === 'datestamp_day_full') || (property === 'startdate_day_full') || (property === 'submitdate_day_full')) {
                    line.width = 145;
                    line.hide = true;
                };

                if ((property === 'datestamp_time') || (property === 'startdate_time') || (property === 'submitdate_time')) {
                    line.width = 145;
                    line.hide = true;
                };
