...
angular.module('optinomicDataModule')
    .service('dataService', function($q, $filter, apiService) {
        ...
        dataService.getSurveyResponses = function(stay_id) {
            ...
            if (have_stay) {
                var api_call = "/stays/" + stay_id + "/survey_responses/" + helpers.getAppID();
            } else {
                var api_call = "/patients/" + helpers.getPatientID() + "/survey_responses/" + helpers.getAppID();
            };
        ...
        dataService.getMainAppData = function() {
            ...
            return_data.params.stay_id = dataService.getStayID();
            ...
            api = dataService.getSurveyResponses(dataService.getStayID());
        ...
        dataService.getAnnotationsData = function(api_direction, node_tree) {
            ...
            aPromise.then(function(data) {
                ...
                // console.log('(!) getAnnotationsData - Success', node_tree, return_data);

                // ToDo: Loop return_data and return just data per stay / all if !have_stay.


                var stay_id = dataService.getStayID();
                stay_id = stay_id === NaN ? 0 : stay_id;

                if ((stay_id > 0) && isArray(return_data)) {
                    // Return data - current stay
                    return_data.forEach(function(item, myindex) {
                        if (item.save_stamp.getStayID === stay_id) {
                            return_array.push(item);
                        };
                    });

                } else {
                    // Return all data - all stay
                    return_array = return_data;
                };
        ...
        dataService.saveAnnotationsData = function(api_direction, node_tree, json_value) {
            ...
            aPromise.then(function(current_data) {
                ...
                if (isArray(full_data[node_tree])) {
                    full_data[node_tree].forEach(function(item, myindex) {
                        if ((item.save_stamp === undefined) || (item.save_stamp === NaN)) {
                            ...
                            item.save_stamp = {};
                            item.save_stamp.getPatientID = dataService.getPatientID();
                            item.save_stamp.getAppName = dataService.getAppName();
                            item.save_stamp.getAppID = dataService.getAppID();
                            item.save_stamp.getApiURL = dataService.getApiURL();
                            item.save_stamp.getUserID = dataService.getUserID();
                            item.save_stamp.getStayID = dataService.getStayID();

                            var date = new Date();
                            item.save_stamp.date = {
