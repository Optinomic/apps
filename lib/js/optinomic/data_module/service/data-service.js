...
angular.module('optinomicDataModule')
    .service('dataService', function($q, $filter, apiService) {
        ...
        dataService.getSurveyResponses = function(stay_id) {
            var have_stay = stay_id === NaN ? false : true;

            var api_call = "";
            if (have_stay) {
                api_call = "/patients/" + helpers.getPatientID() + "/survey_responses/" + helpers.getAppID();
            } else {
                api_call = "/stays/" + stay_id + "/survey_responses/" + helpers.getAppID();
            };

            console.log('(! API) - getSurveyResponses: ', stay_id, api_call);
        ...
        dataService.getMainAppData = function() {
            ...
            api = dataService.getSurveyResponses(helpers.getStayID());
