...
angular.module('optinomicDataModule')
    .service('dataService', function($q, $filter, apiService) {
        ...
        dataService.getUserAnnotationsData = function(app_id, node_tree) {
            var deferred = $q.defer();
            var return_data = {};
            ...
            aPromise.then(function(data) {
                ...
                console.log('(1) getUserAnnotationsData', data, app_id, node_tree);

                console.log('(2) getUserAnnotationsData', data.app[app_id]);

                return_data = data.app[app_id][node_tree] === undefined ? {} : data.app[app_id][node_tree];
                return_data = JSON.parse(JSON.stringify(return_data));


                console.log('(Return) getUserAnnotationsData', return_data);
                deferred.resolve(return_data);
        ...
        dataService.putUserAnnotationsData = function(app_id, node_tree, json_value) {
            ...
            mySettings.then(function(current_data) {
                ...
                // Save and proceed.
                var api = dataService.putUserAnnotations(JSON.stringify(full_data));
                var aPromise = dataService.getData(api);
