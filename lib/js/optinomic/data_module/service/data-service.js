...
angular.module('optinomicDataModule')
    .service('dataService', function($q, $filter, apiService) {
        ...
        dataService.getPatientAnnotationsData = function(node_tree) {
            var deferred = $q.defer();
            var return_data = {};
            ...
            aPromise.then(function(data) {
                ...
                //console.log('(!): getPatientAnnotationsData', data, data[node_tree], return_data);
                ...
                // Error
                deferred.reject(return_data);
                console.log('ERROR: getPatientAnnotationsData', error);
            });

            return deferred.promise;
        };

        dataService.putPatientAnnotationsData = function(node_tree, json_value) {

            var deferred = $q.defer();
            var return_data = {};


            var mySettings = dataService.getPatientAnnotations();
            mySettings.then(function(current_data) {


                if (node_tree === 'clear') {
                    // Overwrite Settings with given json_value
                    full_settings = json_value;
                } else {
                    // Save given json_value in given node_tree
                    var full_settings = current_data;
                    full_settings[node_tree] = json_value;
                    //full_settings = JSON.stringify(full_settings);
                };


                var api = dataService.putPatientAnnotations(JSON.stringify(full_settings));
                var aPromise = dataService.getData(api);

                aPromise.then(function(data) {

                    //console.log('(+) putPatientAnnotationsData', data, full_settings);
                    deferred.resolve(return_data);

                }, function(error) {
                    // Error
                    deferred.reject(error);
                    console.log('ERROR: putPatientAnnotationsData', error);
                });
        ...
        dataService.getUserAnnotationsData = function(node_tree) {
            var deferred = $q.defer();
            var return_data = {};
            ...
            aPromise.then(function(data) {
                ...
                // Error
                deferred.reject(return_data);
                console.log('ERROR: getUserAnnotationsData', error);
            });

            return deferred.promise;
        };

        dataService.putUserAnnotationsData = function(app_id, node_tree, json_value) {
            ...
            var my_data = {};
            my_data[node_tree] = json_value;

            console.log('(0) putUserAnnotationsData START', my_data);



            var mySettings = dataService.getUserAnnotations();
            mySettings.then(function(current_data) {

                full_data = current_data.data;


                console.log('(1) putUserAnnotationsData START', full_data, current_data);
                ...
                if (node_tree === 'clear') {
                    ...
                    // Save given json_value in given node_tree
                    full_settings = full_data;
                };

                console.log('(2) putUserAnnotationsData', full_settings);


                full_settings.app = full_settings.app === undefined ? {} : full_settings.app;
                full_settings.app[app_id] = full_settings.app[app_id] === undefined ? {} : full_settings.app[app_id];
                full_settings.app[app_id][node_tree] = json_value;

                console.log('(3) putUserAnnotationsData', full_settings);

                var api = dataService.putUserAnnotations(JSON.stringify(full_settings));
                var aPromise = dataService.getData(api);
