...
const reducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'GET_CURRENT_PATIENT_COMPLETE':
            return Object.assign({}, state, {
                loading: false,
                current_patient: action.data
            });

        case 'GET_CURRENT_PATIENT_STAYS_COMPLETE':
            return Object.assign({}, state, {
                loading: false,
                current_patient_stays: action.data
            });
    ...
    actions: {
        ...
        actionGetSurveyResponses: function() {
            return function(dispatch) {
                ...
                if (current_stay_id) {
                    const api_url = '/stays/' + current_stay_id + '/survey_responses/' + module_identifier + '/full';
                } else {
                    const api_url = '/patients/' + helpers.getPatientID() + '/survey_responses/' + module_identifier + '/full';
                };
