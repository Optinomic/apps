                                ...
                                current_calculation_top.value.forEach(function(current_calculation, calculationID) {
                                    ...
                                    if (variant_response) {
                                        ...
                                        if (JSON.stringify(calc_resp) === JSON.stringify(return_obj.response)) {
                                            ...
                                            // Find if all Propertys are the same.

                                            if ("TMTAError" in calc_resp) {
                                                // TMT - Special
                                                console.error('DEBUG HERE ::', calc_resp, return_obj.response, current_calculation);

                                                if ((calc_resp.TMTAError === parseInt(return_obj.response.TMTAError)) &&
                                                    (calc_resp.TMTATime === parseInt(return_obj.response.TMTATime)) &&
                                                    (calc_resp.TMTBError === parseInt(return_obj.response.TMTBError)) &&
                                                    (calc_resp.TMTBTime === parseInt(return_obj.response.TMTBTime)) &&
                                                    (calc_resp.Ausbildungsjahre === parseInt(return_obj.response.Ausbildungsjahre)) &&
                                                    (calc_resp.Messzeitpunkt === parseInt(return_obj.response.Messzeitpunkt))
                                                ) {

                                                    var merged_object = Object.assign(calc_resp, return_obj.response);
                                                    console.error('FOUND ::', merged_object);
                                                    ...
                                                    console.error('1 NOT FOUND ::', calc_resp.TMTAError, parseInt(return_obj.response.TMTAError));
                                                    console.error('2 NOT FOUND ::', calc_resp.TMTATime, parseInt(return_obj.response.TMTATime));
                                                    console.error('3 NOT FOUND ::', calc_resp.Ausbildungsjahre, parseInt(return_obj.response.Ausbildungsjahre));
                                                    console.error('4 NOT FOUND ::', calc_resp.Messzeitpunkt, parseInt(return_obj.response.Messzeitpunkt));
                                                };
