...
console.log('START :: User-State @ --> ', state);

// START Subscribing to localStorageUser
store.subscribe(function() {
  // Save to localStorage!
  var options = Object.assign({}, UserStore.getOptions());
  var state = Object.assign({}, store.getState());
  Polymer.RenderStatus.afterNextRender(state, function() {
    ...
    if ((options.using) && (options.name !== "unknown")) {
      ...
      if (ApiHelpers.shouldCallNow('save_state', 7)) {
        // All the taxing stuff you do
        setTimeout(function(state) {
          var save_state = Object.assign({}, store.getState());

          Polymer.RenderStatus.afterNextRender(save_state, function() {
